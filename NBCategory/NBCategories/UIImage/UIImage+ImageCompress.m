...
+ (UIImage*)scaleDownRemoteImage:(NSString*)url withSize:(CGSize)newSize{
    //Parse the URL
    NSURL *imageURL = [NSURL URLWithString:url];
    //We init the image with the rmeote data
    UIImage *remoteImage = [UIImage imageWithData:[NSData dataWithContentsOfURL:imageURL]];
    
    return [self thumbnailWithImageWithoutScale:remoteImage size:newSize];
}
